    ...
    <div className="bg-white flex rounded-lg gap-2 p-4 py-2 flex-col justify-between  w-[500px]">
      ...
      <span className="flex gap-2">
        <button
          ...
          className={`${priorityColor} bg-blue-500 text-white rounded-[5px 5px 5px 20px] p-2 w-5 h-5`}
        ></button>
        <button
          onClick={() => setPriority(2)}
          className={`${
            priority > 1 ? priorityColor : "bg-gray-500"
          } bg-blue-500 text-white rounded-[5px 5px 5px 20px] p-2 w-5 h-5`}
        ></button>
        <button
          onClick={() => setPriority(3)}
          className={`${
            priority > 2 ? priorityColor : "bg-gray-500"
          } bg-blue-500 text-white rounded-[5px 5px 5px 20px] p-2 w-5 h-5`}
        ></button>
      </span>
      <button
        className="border-2 border-black p-2 w-full text-red-300 bg-blue-500 w"
        onClick={() => {
          handleToDo();
          setAddTask(false);
        }}
